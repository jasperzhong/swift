...
void pytorch_q8vadd_ukernel__sse2(
  ...
  if
    PYTORCH_QNNP_LIKELY(n >= 8) {
      ...
      const __m128 va_scale = _mm_load_si128(
          (const __m128i*)&quantization_params->sse2.a_scale);
      const __m128 vb_scale = _mm_load_si128(
          (const __m128i*)&quantization_params->sse2.b_scale);
