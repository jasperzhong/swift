...
void pytorch_q8vadd_ukernel__neon(
  ...
  if
    ...
    for (; n != 0; n--) {
      ...
      float32x2_t vxa_f = vcvtq_f32_s32(vget_low_s32(vmovl_s16(vxa)));
      float32x2_t vxb_f = vcvtq_f32_s32(vget_low_s32(vmovl_s16(vxb)));

      float32x2_t vacc_f = vmulq_f32(vxa_f, va_multiplier);
      vacc_f = vmlaq_f32(vacc_f, vxb_f, vb_multiplier);

#ifdef __aarch64__
      int32x2_t vacc = vcvtnq_s32_f32(vacc_f);
      ...
      vy = vmax_u8(vy, vget_low_u8(vy_min));
#else
      vacc_f = vminq_f32(vmaxq_f32(vacc_f, vfmin), vfmax);

      int32x2_t vacc = vsubq_s32(
