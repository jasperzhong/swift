...
struct BenchmarkCache {
  ...
  BenchmarkCache(std::string name) {
    filename = "cudnn_conv_v7_benchmark_" + name + ".cache";
  ...
  void insert(const ConvolutionParams& params, const T& results) {
    ...
    // serialize the key-value to the file
    int num_bytes = sizeof(ConvolutionParams) + sizeof(T);
    std::vector<char> buffer(num_bytes);
...
struct Workspace {
  Workspace(size_t size) : size(size), data(NULL) {
    ...
    TORCH_CHECK_WITH(
        CUDAOutOfMemoryError, size < 1_TiB, "Not enough memory for workspace!");
...
size_t getMaxWorkspaceSize(
  ...
  c10::cuda::CUDACachingAllocator::cacheInfo(
      device, &tmp_bytes, &max_block_size);
