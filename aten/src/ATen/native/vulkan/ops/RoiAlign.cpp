...
at::Tensor roi_align(
  ...
  std::cout << "XXX roi_align Vulkan rois.sizes():" << rois_.sizes() << std::endl;
  api::Context* const context = api::context();

  const vTensor& v_input = convert(input);
  const auto rois = rois_.is_vulkan() ? rois_ : rois_.vulkan();
...
TORCH_LIBRARY_IMPL(_caffe2, Vulkan, m) {
  m.impl("_caffe2::RoIAlign", TORCH_FN(roi_align));
}
