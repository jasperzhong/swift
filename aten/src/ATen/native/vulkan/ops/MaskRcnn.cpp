...
std::tuple<at::Tensor, at::Tensor> GenerateProposalsVulkanKernel(
    const at::Tensor& scores_,
    const at::Tensor& bbox_deltas_,
    const at::Tensor& im_info_tensor_,
    const at::Tensor& anchors_tensor_,
  ...
  auto out_cpu = GenerateProposalsCPUKernel(
      scores_.cpu(),
      bbox_deltas_.cpu(),
      im_info_tensor_.cpu(),
      anchors_tensor_.cpu(),
...
at::Tensor ROIAlignForwardCPUKernel(
  ...
  std::cout << "XXX ROIAlignForwardCPUKernel" << std::endl;
...
at::Tensor ROIAlignForwardVulkanKernel(
    const at::Tensor& features,
    const at::Tensor& rois,
    std::string order,
    double spatial_scale,
  ...
  std::cout << "XXX ROIAlignForwardVulkanKernel 0" << std::endl;
  auto out = ROIAlignForwardCPUKernel(
    features.cpu(),
    rois.cpu(),
    order,
    spatial_scale,
  ...
  std::cout << "XXXY ROIAlignForwardVulkanKernel 1 out.sizes():" << out.sizes() << std::endl;
  //auto out_vulkan = out.vulkan();
  auto sizes = out.sizes();
  auto out_vulkan = 
    sizes[0] == 0 
    ? at::empty({1, sizes[1], sizes[2], sizes[3]}, at::device(at::kVulkan).dtype(at::kFloat))
    : at::empty(out.sizes(), at::device(at::kVulkan).dtype(at::kFloat));
  std::cout << "XXXY out_vulkan.is_vulkan():" << out_vulkan.is_vulkan() << std::endl;
  std::cout << "XXXY (at::kVulkan == out_vulkan.device().type()):"
    << (at::kVulkan == out_vulkan.device().type()) << std::endl;
  return out_vulkan;
}
...
m.impl(
    "_caffe2::GenerateProposals",
    TORCH_FN(at::native::GenerateProposalsVulkanKernel));
/*m.impl(
    "_caffe2::RoIAlign",
    TORCH_FN(at::native::ROIAlignForwardVulkanKernel));*/
m.impl(
    "_caffe2::HeatmapMaxKeypoint",
    TORCH_FN(at::native::HeatmapMaxKeypointVulkanKernel));
m.impl(
    "_caffe2::BBoxTransform",
    TORCH_FN(at::native::BBoxTransformVulkanKernel));
m.impl(
    "_caffe2::BoxWithNMSLimit",
    TORCH_FN(at::native::BoxWithNMSLimitVulkanKernel));
}
