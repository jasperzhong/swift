...
at::Tensor HeatmapMaxKeypointCPUKernel(
  ...
  auto heatmaps_in = heatmaps_in_.contiguous();
  const auto bboxes_in = bboxes_in_.contiguous();
  std::cout << "XXX bboxes_in.sizes():" << bboxes_in.sizes() << std::endl;
  std::cout << "XXX heatmaps_in.sizes():" << heatmaps_in.sizes() << std::endl;
  if (bboxes_in.size(0) == 0) {
    auto hsizes = heatmaps_in.sizes();
    heatmaps_in = at::empty({0, hsizes[1], hsizes[2], hsizes[3]});
  }
...
torch::Tensor HeatmapMaxKeypointVulkanKernel(
    const torch::Tensor& heatmaps_in,
    const torch::Tensor& bboxes_in,
    bool should_output_softmax_,
    c10::optional<std::vector<torch::Tensor>>
) {
  return HeatmapMaxKeypointCPUKernel(
    heatmaps_in.cpu(),
    bboxes_in.cpu(),
    should_output_softmax_,
    c10::nullopt
  );
}
