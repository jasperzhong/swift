...
std::tuple<at::Tensor, at::Tensor, at::Tensor, at::Tensor, at::Tensor, at::Tensor> BoxWithNMSLimitCPUKernel(
  ...
  auto tscores = tscores_.contiguous();
  const auto tboxes = tboxes_.contiguous();
  if (tboxes.size(0) == 0) {
    tscores = at::empty({0, tscores.size(1)});
  }
  ...
  std::cout << "XXX tboxes.sizes():" << tboxes.sizes() << std::endl;
  std::cout << "XXX tscores.sizes():" << tscores.sizes() << std::endl;
...
std::tuple<torch::Tensor, torch::Tensor, torch::Tensor, torch::Tensor, torch::Tensor, torch::Tensor> BoxWithNMSLimitVulkanKernel(
  ...
  return BoxWithNMSLimitCPUKernel(
    tscores.cpu(),
    tboxes.cpu(),
    tbatch_splits.cpu(),
    score_thres_,
