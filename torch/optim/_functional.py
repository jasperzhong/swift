...
def bertadam(params: List[Tensor],
          grads: List[Tensor],
          exp_avgs: List[Tensor],
          exp_avg_sqs: List[Tensor],
          max_exp_avg_sqs: List[Tensor],
          ...
          *,
          ...
          weight_decay: float,
          eps: float):
    r"""Functional API that performs BertAdam algorithm computation.

    See :class:`~torch.optim.BertAdam` for details.
    """
    for i, param in enumerate(params):
        grad = grads[i]
        exp_avg = exp_avgs[i]
...
def undo_bertadam(params: List[Tensor],
               grads: List[Tensor],
               exp_avgs: List[Tensor],
               exp_avg_sqs: List[Tensor],
               state_steps: List[int],
