...
c10::IValue preprocess(
  ...
  // Test that method_compile_spec contains the necessary keys and Tensor/TensorList
  // input
  c10::IValue inp;
  std::string error = "";
  if (!method_compile_spec.contains("forward")) {
    ...
    if (!innerDict.isGenericDict() || !innerDict.toGenericDict().contains("inputs")) {
      error =
      ...
      if (!inp.isTensor() && !inp.isTensorList()) {
        error =
            "compile_spec does not contain either a Tensor or TensorList, under it's \"inputs\" key, under \"forward\" key.";
      }
    }
  }
  if (error.size() != 0) {
