...
class HermeticExporter:
    ...
    resolves relative references to qualified module names, and calls :method:`save_module`
    ...
    def save_source_file(self, module_name: str, file_or_directory: str, dependencies=True):
        ...
        """
        if Path(file_or_directory).is_dir():
            ...
            is_package = file_or_directory.endswith('/__init__.py')
            self.save_source_string(module_name, _read_file(file_or_directory), is_package, dependencies, file_or_directory)

    def save_source_string(self, module_name: str, src: str, is_package: bool = False, dependencies: bool = True, orig_file_name:str = None):
        ...
        if dependencies:
            ...
            for dep_module_name in dep_list:
                try:
                    self._import_module(dep_module_name)
                    exists = True
                except ModuleNotFoundError:
                    exists = False
                # its possible this module is only optionallly used and loaded,
                # so we only include modules that we actually can succesfully load
                if exists:
                    self.save_module(dep_module_name)

    def _get_source_of_module(self, module: types.ModuleType) -> str:
        filename = getattr(module, '__file__', None)
        if filename is None or not filename.endswith('.py'):
            raise ValueError(f'cannot save source for module "{module.__name__}" because its source file is not python source "{filename}". If this module should be an external dependency, call allow_module to add it as one before writing files that depend on it.')

        if hasattr(module.__loader__, 'source_for_module'):
            # e.g. provided by HermeticImporter to get source from zip files
            return module.__loader__.source_for_module(module)
        ...
        raise ValueError(f'cannot save source for module "{module.__name__}" because its source file "{filename}" could not be found.')
    ...
    def save_module(self, module_name: str, dependencies=True):
        ...
        module object, and then using its `__file__` attribute to find the source code. This will only save code if it does not
        already exist in the package (either as an external module or saved source code).
        ...
        if self._can_implicitly_extern(root_name):
            if self.verbose:
                print(f'implicitly adding {root_name} to external modules since it is part of the standard library and is a dependency.')
    ...
    def _create_pickler(self, data_buf):
        if self.importers == [importlib.import_module]:
            ...
            return CustomImportPickler(self._import_module, data_buf)
    ...
    def save_pickle(self, package: str, resource: str, obj: Any, dependencies: bool = True):
        ...
        to reconstruct them and save the relevant code.  To be able to save an object where `type(obj).__name__` is `my_module.MyObject`,
    ...
    def save_text(self, package: str, resource: str, text: str):
        ...
        """
        return self.save_binary(package, resource, text)
    ...
    def extern_module(self, module_name: str):
        """Include `module` in the list of external modules the package can import. This will prevent dependency discover from saving
        it in the package. The importer will load an external module directly from the standard import system. Code for extern
        modules must also exist in the process loading the package.
    ...
    def mock_module(self, module_name: str):
        ...
        find files that are imported by model files but whose functionality is never used (e.g. custom serialization code or training helpers).
    ...
    def close(self):
        ...
        It is preferable to use resource guard syntax instead:
            ...
            with HermeticExporter("file.zip") as e:
                ...
        """
    ...
    def _can_implicitly_extern(self, module_name: str):
        print(module_name)
        return module_name == 'torch' or (module_name not in _DISALLOWED_MODULES and _is_builtin_or_stdlib_module(self._import_module(module_name)))
