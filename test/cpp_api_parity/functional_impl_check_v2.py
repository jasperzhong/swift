...
def compute_functional_name(test_params_dict):
  if 'cpp_options_arg' in test_params_dict:
...
def compute_cpp_function_call(test_params_dict, arg_dict, functional_name):
  ...
  elif 'cpp_options_arg' in test_params_dict:
    cpp_forward_args_symbols = [arg_name for arg_name, _ in arg_dict['input'] + arg_dict['target'] + arg_dict['extra_args']]
    return 'F::{}({}, {})'.format(functional_name, ", ".join(cpp_forward_args_symbols), test_params_dict['cpp_options_arg'])
  else:
    raise RuntimeError(
      "`cpp_options_arg` or `cpp_function_call` entry must be present in test params dict: {}".format(
        test_params_dict))
