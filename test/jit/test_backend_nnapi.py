...
compile_spec must use the following format: {"forward": {"inputs": at::Tensor}} OR {"forward": {"inputs": c10::List<at::Tensor>}}"""
        ...
        with self.assertRaisesRegex(RuntimeError, "compile_spec does not contain a dictionary with an \"inputs\" key, under it's \"forward\" key." + errorMsgTail):
            torch._C._jit_to_backend("nnapi", traced, compile_spec)
        ...
        with self.assertRaisesRegex(RuntimeError, "compile_spec does not contain either a Tensor or TensorList, under it's \"inputs\" key, under \"forward\" key." + errorMsgTail):
            torch._C._jit_to_backend("nnapi", traced, compile_spec)
        compile_spec = {"forward":{"inputs": [1]}}
        with self.assertRaisesRegex(RuntimeError, "compile_spec does not contain either a Tensor or TensorList, under it's \"inputs\" key, under \"forward\" key." + errorMsgTail):
            torch._C._jit_to_backend("nnapi", traced, compile_spec)
