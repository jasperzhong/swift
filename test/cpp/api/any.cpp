...
TEST_F(AnyModuleTest, WrongNumberOfArguments) {
  ...
  ASSERT_THROWS_WITH(
      ...
      "please make sure the forward() method is declared using `FORWARD_HAS_DEFAULT_ARGS` macro.");
  ASSERT_THROWS_WITH(
      any.forward(5),
...
struct M_default_arg_in_forward : torch::nn::Module {
...
TEST_F(AnyModuleTest, PassingArgumentsToModuleWithDefaultArgumentsInForwardMethod) {
  AnyModule any(M_default_arg_in_forward{});
  ASSERT_EQ(any.forward<double>(1), 6.0);
  ASSERT_EQ(any.forward<double>(1, 3), 7.0);
  ASSERT_EQ(any.forward<double>(1, 3, 5.0), 9.0);
  ASSERT_THROWS_WITH(
      any.forward(),
      "M_default_arg_in_forward's forward() method expects at least 1 argument(s) and at most 3 argument(s), but received 0.");
  ASSERT_THROWS_WITH(
      any.forward(1, 2, 3.0, 4),
      "M_default_arg_in_forward's forward() method expects at least 1 argument(s) and at most 3 argument(s), but received 4.");
}
